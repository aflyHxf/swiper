<template>
    <div class="confirm" @touchmove.prevent='1' v-if='confirm.length'>
        ...
        <div class="content_warp" flex="main:center cross:center">
            <div class="content">
                <div class="effect" :class="{enter:show,leave:!show}" flex="dir:top box:last">
                    <h2 class="icon_warn2">{{confirm[0].title || '温馨提示'}}</h2>
                    <div class="text" v-html="confirm[0].text|| '请确认'"></div>
                    <div class="btns" flex='main:center cross:center'>
                        <div class="btn" @click='execute(confirm[0].okFun)'>{{confirm[0].okName ||'确定'}}</div>
                        <div class="btn" v-if='confirm[0].noFun' @click='execute(confirm[0].noFun)'>{{confirm[0].noName ||'取消'}}</div>
                    </div>
                </div>
            </div>
        </div>
